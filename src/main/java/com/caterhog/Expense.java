...
public class Expense {
    ...
    public static StringBuilder consumptionIns(double x, int t, String y, String data, int user_id, String username) {
        ...
        try {
            ...
            if (data.equals("")) { //если дата не указана
                preparedStatement = connect.prepareStatement("INSERT INTO spend (consumption, category_id, description, user_id, username, op_date) values(?, ?, ?, ?, ?,CURDATE())");
                ...
                preparedStatement = connect.prepareStatement("INSERT INTO spend (consumption, category_id, description, user_id, username, op_date) values(?, ?, ?, ?, ?,STR_TO_DATE(?, '%d.%m.%y'))");
                //preparedStatement.setString(1, "1500");
                preparedStatement.setDouble(1, x);
                preparedStatement.setDouble(2, t);
                preparedStatement.setString(3, y);
            ...
            "UPDATE spend set balance=(select*from(select sum(income)-sum(consumption) from spend where username=?) AS t1) where id=(select *from(select max(id) from spend where username = ?) AS t2 )"
